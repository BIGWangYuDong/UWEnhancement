...
val_ann_file_path = 'zhuang_test.txt'          # txt file for loading images (validate during training process), default = test.txt
test_ann_file_path = 'zhuang_test.txt'         # txt file for loading images, default = test.txt
...
test_pipeling = [dict(type='LoadImageFromFile', gt_type='color', get_gt=False),
                 dict(type='Resize', img_scale=(256, 256), ugan_resize=True),
                 # dict(type='Pad', size_divisor=32, mode='resize'),
                 dict(type='ImageToTensor')]
...
work_dir = './checkpoints/UWGANUNet/1'      #
